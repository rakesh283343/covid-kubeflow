...
set-values:
	...
	kpt cfg set ./upstream/manifests/gcp name ${KF_NAME}
	...
	kpt cfg set ./instance location ${REGION}
	kpt cfg set ./instance gcloud.core.project ${PROJECT_ID}
...
wait-gcp:
	...
	do \
		...
		kubectl --context=$(MGMTCTXT) wait --for=condition=Ready --timeout=600s "$${resource}" -l kf-name=$(NAME) -n $(KF_PROJECT)  \
